...
| 2026-07-20 | Local                        | —         | Pass        | T-140。`@you`→`@guest` seed/migration/docs。投稿API未変更                                          |
| 2026-07-20 | Local                        | —         | Pass        | T-141。未ログイン投稿を Guest 著者へフォールバック。401廃止                                        |
| 2026-07-20 | Local                        | —         | Pass        | T-142。未ログインでも composer 表示＋Guest 投稿案内。ログイン導線維持                              |
| 2026-07-20 | Local                        | —         | Pass        | T-150。`/about` に仕様・技術・実装手順の3セクション。Header導線は T-151                            |
| 2026-07-20 | Local                        | —         | Pass        | T-151。Header に「このシステムについて」→ `/about`。360px共存・focus-visible 維持                  |
...
- [x] **T-140 Guest プロフィールを seed / docs に固定する**
  ...
  実施記録（Local / 2026-07-20）:

  | 確認項目                | 結果 | 備考                                                                                            |
  | ----------------------- | ---- | ----------------------------------------------------------------------------------------------- |
  | seed `@guest` / `Guest` | Pass | `supabase/seed.sql`。UUID `...0001`・`avatar_path=/avatars/you.png` 維持                        |
  | migration               | Pass | `20260720060000_rename_you_to_guest.sql`（UPDATE only。Auth 行は作らない）                      |
  | `author_id` 同一 UUID   | Pass | id 変更なし。既存投稿の FK は壊れない                                                           |
  | docs 固定アカウント表   | Pass | ARCHITECTURE §6.5・SPEC §5.1/§11.7・API 例・ADR-010 を `@guest` に同期                          |
  | 投稿 API 未変更         | Pass | `POST /api/posts` / `createPost` は未改修（未ログインは従来どおり 401。フォールバックは T-141） |

- [x] **T-141 未ログイン投稿を Guest 著者へフォールバックする**
  ...
  | 確認項目                    | 結果 | 備考                                                                          |
  | --------------------------- | ---- | ----------------------------------------------------------------------------- |
  | 未ログイン → Guest 201      | Pass | `resolvePostAuthor` が `FIXED_HUMAN_ACCOUNT_ID` を使用。401 なし              |
  | ログイン中 → セッション著者 | Pass | 既存どおり `sessionUser.id` → `findHumanAccountById`                          |
  | Guest 不在 → 500            | Pass | `INTERNAL_ERROR`。セッション profile 不在も同様                               |
  | AI返信フロー維持            | Pass | `createPost` の mention / 生成ロジックは変更なし。著者は human Account のまま |
  | docs / test                 | Pass | API.md・ARCHITECTURE・SPEC・ADR-010・`posts-post.test.ts` を同期              |

- [x] **T-142 未ログインでも composer を表示し Guest 投稿であることを示す**
  ...
  | 確認項目                         | 結果 | 備考                                                                                     |
  | -------------------------------- | ---- | ---------------------------------------------------------------------------------------- |
  | 未ログインで composer 投稿可     | Pass | `PostComposer` は常時表示。API は T-141 の Guest フォールバック                          |
  | Guest 表示（色以外）             | Pass | Badge「Guest」＋「Guest（@guest）として投稿します」テキスト。composer 内にログインリンク |
  | ログイン中はセッション著者のまま | Pass | `isLoggedIn` 時は Guest 表示なし。著者解決は API / T-141 どおり                          |
  | 360px                            | Pass | `flex-wrap` + `min-w-0`。ラベルと案内が折り返し可能                                      |

### N. About 画面

- [x] **T-150 `/about` に仕様・技術・実装手順ページを実装する**
  ...
  | 確認項目                         | 結果 | 備考                                                         |
  | -------------------------------- | ---- | ------------------------------------------------------------ |
  | `/about` 表示                    | Pass | Server Component。`src/app/about/page.tsx`                   |
  | 3セクション + 戻るリンク         | Pass | コンセプト / 技術スタック / 実装手順。`← タイムラインへ戻る` |
  | docs runtime 読込なし            | Pass | 要約は `src/features/about/components` に静的配置            |
  | `dangerouslySetInnerHTML` 未使用 | Pass | React text のみ                                              |
  | Header リンク                    | —    | T-151 対象（本タスクでは未追加）                             |

- [x] **T-151 Header から About へ遷移できるようにする**
  ...
  | 確認項目                           | 結果 | 備考                                                           |
  | ---------------------------------- | ---- | -------------------------------------------------------------- |
  | Header → `/about`                  | Pass | 「このシステムについて」リンク。メニュー/ドロップダウンなし    |
  | focus-visible                      | Pass | 既存リンクと同パターンの `focus-visible:ring-3`                |
  | 360px でロゴ・ログインと共存       | Pass | ロゴ truncate、About は `text-xs`、ログイン中メール `max-w-20` |
  | docs 要約との突き合わせ（ADR-011） | Pass | `/about` 3セクションは SPEC §11.8 と一致。本タスクは導線のみ   |
